...
from inverse_irl.models.architectures import feedforward_energy, relu_net
from inverse_irl.models.tf_util import discounted_reduce_sum
from inverse_irl.utils.general import TrainingIterator
from inverse_irl.utils.hyperparametrized import Hyperparametrized
...
class ImitationLearning(object, metaclass=Hyperparametrized):
    ...
    def _make_param_ops(self, vs):
        self._params = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, scope=vs.name)
        assert len(self._params)>0
        self._assign_plc = [tf.compat.v1.placeholder(tf.float32, shape=param.get_shape(), name='assign_%s'%param.name.replace('/','_').replace(':','_')) for param in self._params]
        self._assign_ops = [tf.compat.v1.assign(self._params[i], self._assign_plc[i]) for i in range(len(self._params))]
...
class AIRLStateAction(SingleTimestepIRL):
    ...
    def __init__(self, env_spec, expert_trajs=None,
        ...
        # build energy model
        with tf.compat.v1.variable_scope(name) as _vs:
            # Should be batch_size x T x dO/dU
            self.obs_t = tf.compat.v1.placeholder(tf.float32, [None, self.dO], name='obs')
            self.act_t = tf.compat.v1.placeholder(tf.float32, [None, self.dU], name='act')
            self.labels = tf.compat.v1.placeholder(tf.float32, [None, 1], name='labels')
            self.lprobs = tf.compat.v1.placeholder(tf.float32, [None, 1], name='log_probs')
            self.lr = tf.compat.v1.placeholder(tf.float32, (), name='lr')

            obs_act = tf.concat([self.obs_t, self.act_t], axis=1)
            with tf.compat.v1.variable_scope('discrim') as dvs:
                with tf.compat.v1.variable_scope('energy'):
                ...
                discrim_vars = tf.compat.v1.get_collection('reg_vars', scope=dvs.name)
...
class GAN_GCL(TrajectoryIRL):
    ...
    def __init__(self, env_spec, expert_trajs=None,
        ...
        with tf.compat.v1.variable_scope(name) as vs:
            # Should be batch_size x T x dO/dU
            self.obs_t = tf.compat.v1.placeholder(tf.float32, [None, None, self.dO], name='obs')
            self.act_t = tf.compat.v1.placeholder(tf.float32, [None, None, self.dU], name='act')
            self.traj_logprobs = tf.compat.v1.placeholder(tf.float32, [None, None], name='traj_probs')
            self.labels = tf.compat.v1.placeholder(tf.float32, [None, 1], name='labels')
            self.lr = tf.compat.v1.placeholder(tf.float32, (), name='lr')
            ...
            with tf.compat.v1.variable_scope('discrim') as vs2:
                self.energy = discrim_arch(obs_act, **discrim_arch_args)
