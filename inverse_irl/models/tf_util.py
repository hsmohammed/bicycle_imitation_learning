...
def linear(X, dout, name, bias=True):
    with tf.compat.v1.variable_scope(name):
        ...
        W = tf.compat.v1.get_variable('W', shape=(dX, dout))
        tf.compat.v1.add_to_collection(REG_VARS, W)
        if bias:
            b = tf.compat.v1.get_variable('b', initializer=tf.constant(np.zeros(dout).astype(np.float32)))
        else:
            b = 0
