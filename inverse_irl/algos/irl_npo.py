...
from inverse_irl.algos.penalty_lbfgs_optimizer import PenaltyLbfgsOptimizer
from inverse_irl.algos.irl_batch_polopt import IRLBatchPolopt
...
class IRLNPO(IRLBatchPolopt):
    ...
    def init_opt(self):
        ...
        old_dist_info_vars = {
            k: tf.compat.v1.placeholder(tf.float32, shape=[None] * (1 + is_recurrent) + list(shape), name='old_%s' % k)
        ...
        if self.pol_ent_wt > 0:
            if 'log_std' in dist_info_vars:
                ...
                ent = tf.compat.v1.reduce_sum(log_std + tf.compat.v1.log(tf.sqrt(2 * np.pi * np.e)), reduction_indices=-1)
